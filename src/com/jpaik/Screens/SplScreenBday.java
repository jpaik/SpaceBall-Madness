...
public class SplScreenBday implements Screen, InputProcessor {
	...
	private boolean tween2 = false;
	
	public SplScreenBday(TestGame game){
		this.game = game;
	...
	public void show() {
		...
		splashTexture = new Texture("data/HappyBirthday.png"); //import picture
		...
		Tween.to(splashSprite, SpriteTween.ALPHA, 2.75f).target(1).ease(TweenEquations.easeInQuad).repeatYoyo(1, 2.5f).setCallback(cb).setCallbackTriggers(TweenCallback.COMPLETE).start(manager);
	}
