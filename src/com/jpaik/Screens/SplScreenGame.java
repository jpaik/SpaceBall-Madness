...
public class SplScreenGame implements Screen, InputProcessor {
	...
	Music music = Gdx.audio.newMusic(Gdx.files.internal("data/Spacecrusher.ogg"));;
	private boolean tween1 = false;
	
	public SplScreenGame(TestGame game){
		this.game = game;
	...
	public void show() {
		...
		splashTexture = new Texture("data/spaceballpic.png"); //import picture
		...
		Tween.to(splashSprite, SpriteTween.ALPHA, 1.8f).target(1).ease(TweenEquations.easeInQuad).repeatYoyo(1, 2.5f).setCallback(cb).setCallbackTriggers(TweenCallback.COMPLETE).start(manager);
	}
	...
	private void tweenCompleted(){
		...
		game.setScreen(new SplScreenBday(game));
	}
